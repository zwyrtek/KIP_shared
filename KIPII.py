...
class AutoInpaintApp:
    def __init__(self, master):
        self.master = master
        self.master.title("Auto Inpaint - Yellow Mask")

        self.canvas = Canvas(master, width=600, height=400, bg='black')
        self.canvas.pack()

        self.label = Label(master, text="Upload image with yellow mask (e.g., yellow rectangle).")
        self.label.pack()
    ...
    def upload_image(self):
        ...
        self.label.config(text="Yellow mask detected. You may now inpaint.")

    def inpaint_image(self):
        if self.image is None or self.mask is None:
            return
        method = cv2.INPAINT_TELEA if self.alg_var.get() == 'TELEA' else cv2.INPAINT_NS
        inpainted = cv2.inpaint(self.image, self.mask, 3, method)
        self.show_image(inpainted)
        self.label.config(text=f"Inpainting completed using {self.alg_var.get()}.")
